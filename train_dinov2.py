...
class DINOv2Classifier(nn.Module):
    """Frozen DINOv2 backbone + trainable Linear head.

    Uses concat of CLS tokens from the last ``concat_layers`` transformer
    blocks for richer frozen features (e.g. concat_layers=2 doubles dims).
    """
    ...
    def forward(self, x):
        with torch.no_grad():
            ...
            for i, blk in enumerate(self.backbone.blocks):
                x = blk(x)
                if i >= len(self.backbone.blocks) - self.concat_layers:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train DINOv2 frozen-feature classifier for Kaggle image classification."
    )
    parser.add_argument("--data_dir", type=str, default=".")
    parser.add_argument("--epochs", type=int, default=30)
    parser.add_argument("--batch_size", type=int, default=128)
    parser.add_argument("--lr", type=float, default=1e-3,
                        help="Higher LR than fine-tuning: we train only the linear head.")
    parser.add_argument("--weight_decay", type=float, default=1e-4)
    parser.add_argument("--image_size", type=int, default=224)
    parser.add_argument("--model_name", type=str, default="dinov2_vits14",
                        choices=["dinov2_vits14", "dinov2_vitb14", "dinov2_vitl14",
                                 "convnext_tiny"],
                        help="DINOv2 variants use frozen features; convnext_tiny is full fine-tune.")
    ...
    parser.add_argument("--concat_layers", type=int, default=2,
                        help="Number of final transformer layers whose CLS tokens are concatenated "
                             "(1=last only, 2=last two).")
    parser.add_argument("--train_image_folder", type=str, default="train_images",
                        help="Subfolder under data_dir containing train images.")
    parser.add_argument("--labels_csv", type=str, default="train_labels.csv",
                        help="Filename of the labels CSV under data_dir.")
...
def build_model(model_name: str, num_classes: int, pretrained: bool = True,
                concat_layers: int = 2) -> nn.Module:
    if model_name.startswith("dinov2"):
        return _build_dinov2(model_name, num_classes, concat_layers=concat_layers)
...
def _build_dinov2(variant: str, num_classes: int, concat_layers: int = 2) -> DINOv2Classifier:
    base_dim = DINOV2_DIMS[variant]
    in_features = base_dim * concat_layers
    ...
    model = DINOv2Classifier(backbone, in_features, num_classes, concat_layers=concat_layers)
    return model
...
def make_loaders(args, train_df, val_df, train_targets, val_targets, device):
    ...
    train_image_dir = resolve_image_dir(args.data_dir, args.train_image_folder)

    train_dataset = KaggleImageDataset(
        train_df, train_image_dir, label_col="label",
        transform=train_transform, labels=train_targets,
    )
    val_dataset = KaggleImageDataset(
        val_df, train_image_dir, label_col="label",
        transform=val_transform, labels=val_targets,
    )
...
def make_checkpoint(model, optimizer, scheduler, scaler, args, classes,
                    epoch, train_loss, val_loss, val_f1, best_val_loss, best_val_f1):
    return {
        ...
        "concat_layers": getattr(args, "concat_layers", 2),
        "epoch": epoch,
...
def group_stratified_split(df, targets, val_size, seed):
    n_splits = max(2, round(1 / val_size))
    splitter = StratifiedGroupKFold(n_splits=n_splits, shuffle=True, random_state=seed)
    train_idx, val_idx = next(splitter.split(df, targets, df["image_group"].values))
...
def main():
    ...
    classes = [v.item() if hasattr(v, "item") else v for v in label_encoder.classes_]
    num_classes = len(classes)
    class_counts = np.bincount(targets, minlength=num_classes)

    # Resolve image dir early for group-aware split
    train_image_dir = resolve_image_dir(args.data_dir, args.train_image_folder)
    df = add_image_groups(df, train_image_dir)

    keep_mask = df["image_group"].notna() & ~df["image_group"].astype(str).str.startswith("missing_") & ~df["image_group"].astype(str).str.startswith("broken_")
    n_excluded = (~keep_mask).sum()
    if n_excluded > 0:
        logger.info("Excluding %d images with missing/broken files.", n_excluded)
        df = df[keep_mask].reset_index(drop=True)
        targets = targets[keep_mask.values]

    train_df, val_df, train_targets, val_targets = group_stratified_split(
        df, targets, val_size=args.val_size, seed=args.seed,
    )
    ...
    train_loader, val_loader, _ = make_loaders(
        args, train_df, val_df, train_targets, val_targets, device,
    )
    ...
    model = build_model(args.model_name, num_classes=num_classes, pretrained=True,
                        concat_layers=args.concat_layers).to(device)

    if args.model_name.startswith("dinov2"):
        trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
        total = sum(p.numel() for p in model.parameters())
        base_dim = DINOV2_DIMS[args.model_name]
        logger.info("DINOv2 backbone frozen. Trainable params: %s / %s (%.1f%%)",
                    trainable, total, 100 * trainable / max(total, 1))
        logger.info("Concat last %d CLS tokens → head input dim = %d",
                    args.concat_layers, base_dim * args.concat_layers)
    ...
    with metrics_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=[
            "epoch", "train_loss", "val_loss", "val_f1", "lr", "is_best", "bad_epochs",
        ])
        writer.writeheader()
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            checkpoint = make_checkpoint(
                ...
                args=args, classes=classes, epoch=epoch,
            ...
            logger.info(
                ...
                "lr=%.6g | best_f1=%s | bad_epochs=%d/%d",
                epoch, args.epochs, train_loss, val_loss, val_f1,
                current_lr, is_best, early_stopping.bad_epochs, args.patience,
            )
            scheduler.step()

            if should_stop:
                stopped_early = True
                logger.info("Early stopping at epoch %d. Best val_f1=%.6f, best val_loss=%.6f",
                            epoch, best_val_f1, best_val_loss)
                break

    if not best_path.is_file():
        logger.warning("Best checkpoint was not created; copying last checkpoint as best.")
    ...
    from predict_dinov2 import run_prediction
